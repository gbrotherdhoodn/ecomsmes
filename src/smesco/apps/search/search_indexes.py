...
from apps.templatetags.utils_tags import get_discount_price
...
class ProductIndex(indexes.SearchIndex, indexes.Indexable):
    ...
    structure = indexes.CharField(model_attr='structure', null=True)
    structure_exact = indexes.CharField(model_attr='structure', null=True, indexed=False)
    ...
    def read_queryset(self, using=None):
        return self.get_model().all.base_queryset()
    ...
    def prepare_category(self, obj):
        if obj.structure == 'child':
            categories = obj.parent.categories.all()
        else:
    ...
    def prepare_attribute(self, obj):
        ...
        if len(attribute_values) > 0:
            ...
            for attribute_value in attribute_values:
                # for option in attribute_value.value_as_html:
                #  Loop over the list of value list that contain the value of the attribute
                if attribute_value.attribute.name.lower() == 'warna':
                    # then append the value to a existing list to be return for the function
                    values.append(attribute_value.value_as_html)
            return values
    ...
    def prepare_price(self, obj):
        ...
        if result:
            price = result.price.incl_tax_incl_discount if result.price.is_tax_known \
                    else result.price.excl_tax_incl_discount
