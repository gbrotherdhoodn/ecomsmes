...
class ProductQuerySet(models.query.QuerySet):
    ...
    def all(self):
        """
        Includes non-canonical products.
        """
        return self.filter()
...
class ProductManager(models.Manager):
    ...
    def exclude_parent(self):
        return self.get_queryset().all().exclude(structure='parent')
...
class AllProductManager(ProductManager):
    """
    Includes non-canonical products
    """

    def get_queryset(self):
        return super(AllProductManager, self).get_queryset().all()
